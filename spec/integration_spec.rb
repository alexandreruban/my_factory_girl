...
RSpec.describe "Integration test" do
  ...
  context "a generated attribute hash" do
    before do
      @attrs = attributes_for(:user, first_name: "Bill")
    end
  ...
  context "a build instance" do
    ...
    it "it does not assign both an association and its foreign key" do
      expect(build(:post, author_id: 1).author_id).to eq(1)
    end
  end
  ...
  context "a custom create" do
    before do
      ...
      FactoryGirl.define do
        factory :user_to_persist, class: User do
          to_create { |user| user.persist }
        end
      end
    end

    it "uses the custom create block instead of save" do
      expect(Factory(:user_to_persist)).to be_persisted
      expect(Factory(:user_to_persist)).to be_a_new_record
    end
  end

  context "a generated stub instance" do
    before do
      @stub = stub(:user, first_name: "Billy")
    end
    ...
    it "assigns associations" do
      expect(stub(:post).author).not_to be_nil
    end
  ...
  context "an attribute generated by a sequence" do
    ...
    context "after the attribute has already generated once" do
      before do
        @another_email = attributes_for(:admin)[:email]
      end
  ...
  context "an attribute generated by an inline sequence" do
    before do
      @username = attributes_for(:admin)[:username]
    end
    ...
    context "after the attribute has already been generated once" do
      before do
        @another_username = attributes_for(:admin)[:username]
      end
